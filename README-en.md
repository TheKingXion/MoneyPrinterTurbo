...
If your AI Agent can read Skill documents and operate a local terminal, ask it to use:

```text
https://raw.githubusercontent.com/harry0703/MoneyPrinterTurbo/main/docs/skill/SKILL.md
```

The Agent installs the project, checks configuration, generates the video, and locates the result. It asks only for required API keys that are missing. This workflow supports macOS and Windows.
...
The CLI defaults to a valid Chinese Edge TTS voice. View all options, pipeline stages, and exit codes with:
...
Use `--video-sources pexels,pixabay,coverr` to combine online providers, `--video-fit-mode cover|blur|contain` to control framing, or `--custom-audio-file` to provide an existing voiceover. Relative material paths resolve from the current working directory; task files are written under `storage/tasks/<task-id>/`.
