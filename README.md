...
如果你的 AI Agent 支持读取 Skill 文档并操作本地终端，可以让它使用：

```text
https://raw.githubusercontent.com/harry0703/MoneyPrinterTurbo/main/docs/skill/SKILL.md
```

Agent 会自动完成安装、配置检查、视频生成和结果定位，仅在缺少必要 API Key 时询问。目前支持 macOS 和 Windows。
...
CLI 默认使用有效的中文 Edge TTS 声音。完整参数、阶段说明和退出码可通过以下命令查看：
...
还可以使用 `--video-sources pexels,pixabay,coverr` 组合在线素材源，使用 `--video-fit-mode cover|blur|contain` 控制画面适配，或用 `--custom-audio-file` 提供已有旁白。相对素材路径从当前工作目录解析；任务文件写入 `storage/tasks/<task-id>/`。
