...
class TestTaskService(unittest.TestCase):
    ...
    def test_save_social_metadata_returns_the_same_payload_written_to_disk(self):
        params = VideoParams(video_subject="Una historia concreta", video_language="es")
        payloads = [
            {"title": "TikTok title", "caption": "TikTok caption", "hashtags": ["#uno"]},
            {"title": "YouTube title", "caption": "YouTube caption", "hashtags": ["#dos"]},
        ]
        with tempfile.TemporaryDirectory() as directory:
            with (
                patch.object(tm.utils, "task_dir", return_value=directory),
                patch.object(tm.llm, "generate_social_metadata", side_effect=payloads) as generate,
            ):
                result = tm.save_social_metadata("task-id", params, "Guion")
            content = (Path(directory) / "METADATOS.md").read_text(encoding="utf-8")

        self.assertEqual(result["tiktok"], payloads[0])
        self.assertEqual(result["youtube_shorts"], payloads[1])
