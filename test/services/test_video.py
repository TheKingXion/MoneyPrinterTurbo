...
class TestVideoService(unittest.TestCase):
    def setUp(self):
        self.original_app_config = dict(config.app)
        config.app["performance_mode"] = "manual"
    ...
    def test_fit_landscape_clip_to_portrait_canvas_modes(self):
        target_size = (90, 160)

        cover_source = vd.ColorClip((160, 90), color=(255, 0, 0)).with_duration(0.1)
        cover = vd._fit_clip_to_canvas(cover_source, *target_size, mode="cover")
        try:
            self.assertEqual(tuple(cover.size), target_size)
            self.assertTrue((cover.get_frame(0)[0, 0] == (255, 0, 0)).all())
        finally:
            vd.close_clip(cover)

        contain_source = vd.ColorClip((160, 90), color=(255, 0, 0)).with_duration(0.1)
        contain = vd._fit_clip_to_canvas(
            contain_source, *target_size, mode="contain"
        )
        try:
            self.assertEqual(tuple(contain.size), target_size)
            self.assertTrue((contain.get_frame(0)[0, 0] == (0, 0, 0)).all())
        finally:
            vd.close_clip(contain)

        blur_source = vd.ColorClip((160, 90), color=(255, 0, 0)).with_duration(0.1)
        blur = vd._fit_clip_to_canvas(blur_source, *target_size, mode="blur")
        try:
            self.assertEqual(tuple(blur.size), target_size)
            self.assertGreater(int(blur.get_frame(0)[0, 0, 0]), 0)
        finally:
            vd.close_clip(blur)
    ...
    def test_combine_videos_uses_concat_for_single_processed_clip(self):
        class _FakeAudioClip:
            duration = 1.0
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            output_file = os.path.join(temp_dir, "combined.mp4")
            with patch.object(vd, "AudioFileClip", return_value=_FakeAudioClip()):
                with patch.object(vd, "_open_video_clip_quietly", return_value=_FakeVideoClip()):
                    with patch.object(vd, "_write_videofile_with_codec_fallback"):
                        with patch.object(vd, "concat_video_clips_with_ffmpeg") as concat_mock:
                            with patch.object(vd, "delete_files"):
                                vd.combine_videos(
                                    combined_video_path=output_file,
                                    video_paths=["single.mp4"],
                                    audio_file="audio.mp3",
                                    video_concat_mode=vd.VideoConcatMode.sequential,
                                )
        ...
        self.assertEqual(concat_mock.call_args.kwargs["max_duration"], 1.0)
    ...
    def test_concat_video_clips_attempts_stream_copy_first(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            clip_file = os.path.join(temp_dir, "temp-clip-1.mp4")
            output_file = os.path.join(temp_dir, "combined.mp4")
            Path(clip_file).write_bytes(b"fake")

            with patch.object(vd.subprocess, "run", side_effect=fake_run) as run:
                used_codec = vd.concat_video_clips_with_ffmpeg(
                    clip_files=[clip_file],
                    output_file=output_file,
                    threads=1,
                    output_dir=temp_dir,
            ...
            command = run.call_args.args[0]
            self.assertEqual(used_codec, "copy")
